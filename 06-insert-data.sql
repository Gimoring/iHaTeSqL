INSERT INTO employers (company_name, company_address, yearly_revenue, is_hiring)
VALUES ('Learning Inc', 'Educationstreet 12, London', 0.87, TRUE);

INSERT INTO employers (company_name, company_address, yearly_revenue)
VALUES ('SuperGuy Inc', 'Slipperystreet 110, Houston', 112.55);

INSERT INTO employers (company_name, company_address, yearly_revenue, is_hiring)
VALUES ('Kebab Kingdom', 'Avocadostreet 5, metz', 6.12, TRUE);

INSERT INTO conversation (user_name, employer_name, message)
VALUES ('Junpil Byeon', 'Learning Inc', 'Hi, I like learning!');